...
public static class L2MicroSplatMutualizer
{
    ...
    [MenuItem("L2/Terrain/Mutualisation/1. Analyser les regions test (aucune ecriture)", false, 180)]
    public static void AnalyseTestRegions()
    {
        Analyse(TestRegions, "regions test");
    }

    [MenuItem("L2/Terrain/Mutualisation/2. Analyser TOUTES les regions (aucune ecriture)", false, 181)]
    public static void AnalyseAllRegions()
    {
        Analyse(EnumerateRegions(), "toutes les regions");
    }

    private static void Analyse(string[] regions, string label)
    {
        ...
        if (settings == null)
        {
            Debug.LogError($"[Mutualisation] Asset de reglages introuvable ({L2TerrainTextureSettings.AssetPath}). "
                           + "Creez-le via L2/Terrain/Textures.");
            return;
        }

        Dictionary<string, string> packOf = BuildSubstitutionMap(settings);
        Dictionary<string, List<string>> layersOf = ReadLayers(EnumerateRegions());

        if (layersOf.Count == 0)
        {
            Debug.LogError($"[Mutualisation] Aucune couche lisible sous {MapsFolder}.");
            return;
        }
        ...
        report.AppendLine($"[Mutualisation] Analyse - {label}.");
        report.AppendLine();
        report.AppendLine($"Tableau partage : {order.Count} packs.");
        report.AppendLine($"  ordre par frequence : {scoreFrequency} textures de controle au total");
        report.AppendLine($"  ordre optimise      : {scoreOptimised}");
        report.AppendLine($"  retenu              : "
                          + (scoreOptimised <= scoreFrequency ? "optimise" : "frequence")
                          + $" (gain {Math.Max(0, scoreFrequency - scoreOptimised)})");
        report.AppendLine();
        report.AppendLine("Ordre retenu :");
        for (int i = 0; i < order.Count; i++)
        {
            report.AppendLine($"  {i,2}  {order[i]}");
        }

        foreach (string region in regions)
        {
            if (!layersOf.TryGetValue(region, out List<string> l2Names))
            {
                report.AppendLine($"  {region} : AUCUNE couche lisible - region ignoree.");
                continue;
            }

            AppendRegion(report, region, l2Names, packOf, indexOf);
        }

        Debug.Log(report.ToString());
    }

    private static void AppendRegion(StringBuilder report, string region, List<string> l2Names,
                                     Dictionary<string, string> packOf, Dictionary<string, int> indexOf)
    {
        // old index -> shared index. Plusieurs anciennes couches peuvent viser
        // la meme tranche : leurs poids devront etre ADDITIONNES, pas deplaces.
        var mapping = new int[l2Names.Count];
        var merged = new Dictionary<int, List<string>>();
        var unresolved = new List<string>();

        for (int i = 0; i < l2Names.Count; i++)
        {
            mapping[i] = -1;

            if (!packOf.TryGetValue(l2Names[i], out string pack) || string.IsNullOrEmpty(pack))
            {
                unresolved.Add($"{i}:{l2Names[i]}");
                continue;
            }

            int idx = indexOf[pack];
            mapping[i] = idx;

            if (!merged.TryGetValue(idx, out List<string> sources))
            {
                sources = new List<string>();
                merged[idx] = sources;
            }
            sources.Add(l2Names[i]);
        }

        int maxIndex = mapping.Where(m => m >= 0).DefaultIfEmpty(-1).Max();
        int layerCount = maxIndex + 1;
        int tier = Tiers.FirstOrDefault(t => t >= layerCount);
        int controlMaps = tier > 0 ? (tier + 3) / 4 : 0;
        int controlsToday = (l2Names.Count + 3) / 4;

        report.AppendLine();
        report.AppendLine($"  {region} : {l2Names.Count} couches -> {layerCount} "
                          + $"(palier _MAX{tier}TEXTURES, {controlMaps} textures de controle "
                          + $"contre {controlsToday} aujourd'hui)");

        foreach (var kv in merged.OrderBy(k => k.Key))
        {
            if (kv.Value.Count > 1)
            {
                report.AppendLine($"      tranche {kv.Key,2} <- FUSION de {string.Join(", ", kv.Value)} "
                                  + "(poids a additionner)");
            }
        }
